...
local function syntax_check(pname)
	...
	local lang = meta and meta.language
	if not lang then
		-- try heuristics based on file layout
		if file_exists(pname .. "/main.py") then
			lang = "python"
		elseif file_exists(pname .. "/main.lua") then
			lang = "lua"
		elseif file_exists(pname .. "/src/main.c") then
			lang = "c"
		elseif file_exists(pname .. "/src/main.cpp") then
			lang = "cpp"
		elseif file_exists(pname .. "/main.ts") or file_exists(pname .. "/src/main.ts") then
			lang = "ts"
		elseif file_exists(pname .. "/main.js") then
			lang = "js"
		elseif file_exists(pname .. "/style.css") or file_exists(pname .. "/main.css") then
			lang = "css"
		elseif file_exists(pname .. "/index.html") then
			lang = "html"
		elseif file_exists(pname .. "/main.go") or file_exists(pname .. "/src/main.go") then
			lang = "go"
		else
			-- detect Java by looking for any .java files in src/
			local srcdir = pname .. "/src"
			local ok, attr = pcall(function()
				return lfs.attributes(srcdir, "mode")
			end)
			...
			if not lang and file_exists(pname .. "/src/main.asm") then
				lang = "assembly"
			end
			if not lang and file_exists(pname .. "/Cargo.toml") then
				lang = "rust"
			end
		end
	end
	...
	if lang == "python" then
		...
		if ok == 0 then
			...
			print("Python syntax errors or python3 not available")
		end
	elseif lang == "lua" then
		local file = pname .. "/main.lua"
		if not file_exists(file) then
			print("No main.lua to check.")
			return
		end
		if os.execute("luac -p " .. string.format("%q", file) .. " >/dev/null 2>&1") == 0 then
			print("Lua syntax OK")
		else
			print("Lua syntax errors or 'luac' not installed")
		end
	elseif lang == "c" then
		local file = pname .. "/src/main.c"
		if not file_exists(file) then
			print("No src/main.c to check.")
			return
		end
		if os.execute("gcc -fsyntax-only " .. string.format("%q", file) .. " >/dev/null 2>&1") == 0 then
			print("C syntax OK")
		else
			print("C syntax errors or gcc not installed")
		end
	elseif lang == "cpp" then
		local file = pname .. "/src/main.cpp"
		if not file_exists(file) then
			print("No src/main.cpp to check.")
			return
		end
		if os.execute("g++ -fsyntax-only " .. string.format("%q", file) .. " >/dev/null 2>&1") == 0 then
			print("C++ syntax OK")
		else
			print("C++ syntax errors or g++ not installed")
		end
	elseif lang == "rust" then
		if not file_exists(pname .. "/Cargo.toml") then
			print("No Cargo.toml found for Rust project")
			return
		end
		if os.execute("cd " .. string.format("%q", pname) .. " && cargo check >/dev/null 2>&1") == 0 then
			print("Rust syntax OK (cargo check)")
		else
			print("Rust check failed or cargo not installed")
		end
	elseif lang == "js" then
		local file = pname .. "/main.js"
		if not file_exists(file) then
			print("No main.js to check.")
			return
		end
		if os.execute("node --check " .. string.format("%q", file) .. " >/dev/null 2>&1") == 0 then
			print("JS syntax OK (node --check)")
		else
			print("JS syntax errors or node doesn't support --check")
		end
	elseif lang == "html" then
		...
		if os.execute("tidy -errors -q " .. string.format("%q", file) .. " >/dev/null 2>&1") == 0 then
			print("HTML looks OK (tidy)")
		else
			print("HTML check failed or 'tidy' not installed; file exists")
		end
	elseif lang == "css" then
		...
		if not file_exists(file) then
			print("No style.css/main.css to check.")
			return
		end
		if os.execute("stylelint " .. string.format("%q", file) .. " >/dev/null 2>&1") == 0 then
			print("CSS looks OK (stylelint)")
		elseif os.execute("csslint " .. string.format("%q", file) .. " >/dev/null 2>&1") == 0 then
			print("CSS looks OK (csslint)")
		else
			print("CSS check failed or no CSS linter installed (stylelint/csslint)")
		end
	elseif lang == "ts" then
		local file = pname .. "/main.ts"
		if not file_exists(file) then
			file = pname .. "/src/main.ts"
		end
		if not file_exists(file) then
			print("No main.ts/src/main.ts to check.")
			return
		end
		if os.execute("tsc --noEmit " .. string.format("%q", file) .. " >/dev/null 2>&1") == 0 then
			print("TypeScript syntax OK (tsc)")
		else
			print("TypeScript check failed or tsc not installed")
		end
	elseif lang == "go" then
		-- run `go build` to validate
		local ok = os.execute("cd " .. string.format("%q", pname) .. " && go build ./... >/dev/null 2>&1")
		if ok == 0 then
			print("Go build OK")
		else
			print("Go build failed or go not installed")
		end
	elseif lang == "java" then
		-- try to find .java files under src/
		local srcdir = pname .. "/src"
		local found = false
		local ok, attr = pcall(function()
			return lfs.attributes(srcdir, "mode")
		...
		if os.execute("javac " .. string.format("%q", srcdir .. "/*.java") .. " >/dev/null 2>&1") == 0 then
			print("Java syntax OK (javac)")
		else
			print("Java compile failed or javac not installed")
		end
	elseif lang == "assembly" then
		local file = pname .. "/src/main.asm"
		if not file_exists(file) then
			print("No src/main.asm to check.")
			return
		end
		if os.execute("nasm -f elf64 -o /dev/null " .. string.format("%q", file) .. " >/dev/null 2>&1") == 0 then
			print("Assembly syntax OK (nasm)")
		else
			print("Assembly syntax errors or nasm not installed")
		end
	else
